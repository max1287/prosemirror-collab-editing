...
`;
  ...
  document.querySelector("#changedoc").addEventListener("click", async e => {
    GET("http://localhost:8000/docs/").then(data => showDocList(e.target, JSON.parse(data)),
  ...
  async function connectFromHash() {
    ...
    if (isID) {
      ...
      connection = window.connection = new EditorConnection(report, "http://localhost:8000/docs/" + isID[1])
